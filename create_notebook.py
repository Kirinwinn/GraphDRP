import nbformat as nbf

nb = nbf.v4.new_notebook()

md1 = """# GraphDRP Automated Batch Prediction

This notebook is an interactive extension of `.py` script to perform batch prediction processing on SMILES data over a designated input folder.
It automatically reads all `.csv` files inside an input folder, extracts graph features using pre-trained `GraphDRP` weights, and saves formatted tables onto a designated output folder."""

code1 = """import os
import csv
import torch
import numpy as np
import pandas as pd
from torch_geometric.data import DataLoader
from tqdm.notebook import tqdm

# Import GraphDRP modules
from models.ginconv import GINConvNet
from models.gat import GATNet
from models.gat_gcn import GAT_GCN
from models.gcn import GCNNet
from utils import TestbedDataset
from preprocess import smile_to_graph"""

md2 = """### 1. Configuration & Directories
Set the global absolute paths for both `input` and `output` folders natively. Make sure your input folder exclusively contains molecular sets stored as `.csv` with a `SMILES` column."""

code2 = """# ================= Configuration ====================
# Modify these absolute paths accordingly
INPUT_FOLDER = "/home/cenking/VsCode/temp/"               # The folder containing your multiple input .csv files
OUTPUT_FOLDER = "/home/cenking/VsCode/GraphDRP/output/"   # The folder where results will be dumped

TARGET_CELL_ID = "1298134"                                # Cosmic ID for HeLa cell
MODEL_WEIGHT_PATH = "/home/cenking/VsCode/GraphDRP/weights/GINConvNet/model_GINConvNet_GDSC.model"
MODEL_CHOICE = 0                                          # 0: GINConvNet
DEVICE_NAME = "cuda:0"                                    # Change to "cpu" if running without GPU

# Ensure the output directory exists
os.makedirs(OUTPUT_FOLDER, exist_ok=True)
# ===================================================="""

md3 = """### 2. Utility Functions Setup"""

code3 = """def load_target_cell_feature(cell_id_target):
    \"\"\"
    Extract feature vector of the target cell line (length 735 0/1 vector).
    Ensure the current working directory points to `GraphDRP` or adjust path absolutely.
    \"\"\"
    pancancer_feature_csv = "/home/cenking/VsCode/GraphDRP/data/PANCANCER_Genetic_feature.csv"
    with open(pancancer_feature_csv, "r") as f:
        reader = csv.reader(f)
        next(reader)
        
        mut_dict = {}
        cell_feature_dict = {}
        
        for item in reader:
            mut = item[5]
            if mut not in mut_dict:
                mut_dict[mut] = len(mut_dict)
                
        f.seek(0)
        next(reader) # skip headers
        for item in reader:
            cell_id = item[1]
            mut = item[5]
            is_mutated = int(item[6])
            
            if cell_id not in cell_feature_dict:
                cell_feature_dict[cell_id] = np.zeros(len(mut_dict))
                
            col = mut_dict[mut]
            cell_feature_dict[cell_id][col] = is_mutated
            
        return cell_feature_dict.get(cell_id_target, None)

def predicting(model, device, loader):
    model.eval()
    total_preds = []
    with torch.no_grad():
        for data in tqdm(loader, desc="Predicting", leave=False):
            data = data.to(device)
            output, _ = model(data)
            total_preds.extend(output.cpu().numpy().flatten())
    return total_preds"""

md4 = """### 3. Load Model and Execute Batch Prediction
This loop goes through all `.csv` documents located inside the `INPUT_FOLDER`."""

code4 = """def run_batch_prediction():
    device = torch.device(DEVICE_NAME if torch.cuda.is_available() else "cpu")
    
    print(f"Loading features for cell line (Cosmic ID: {TARGET_CELL_ID})...")
    cell_feature = load_target_cell_feature(TARGET_CELL_ID)
    if cell_feature is None:
        raise ValueError(f"Features for cell line {TARGET_CELL_ID} not found. Please verify Cosmic ID.")

    print(f"Loading pre-trained model: {MODEL_WEIGHT_PATH}...")
    model_classes = [GINConvNet, GATNet, GAT_GCN, GCNNet]
    model = model_classes[MODEL_CHOICE]().to(device)
    model.load_state_dict(torch.load(MODEL_WEIGHT_PATH, map_location=device, weights_only=False))
    
    csv_files = [f for f in os.listdir(INPUT_FOLDER) if f.endswith('.csv')]
    print(f"Found {len(csv_files)} target file(s) in {INPUT_FOLDER}")
    
    for filename in tqdm(csv_files, desc="Batch File Progress"):
        input_csv_path = os.path.join(INPUT_FOLDER, filename)
        output_csv_path = os.path.join(OUTPUT_FOLDER, filename.replace(".csv", "_evaluated.csv"))
        
        df = pd.read_csv(input_csv_path)
        if 'SMILES' not in df.columns:
            print(f"Warning: Missing 'SMILES' column in {filename}. Skipping...")
            continue
            
        smiles_list = df['SMILES'].tolist()
        smile_graph = {}
        xd_test, xc_test = [], []
        valid_smiles = []
        
        for s in tqdm(smiles_list, desc=f"{filename} converting", leave=False):
            try:
                g = smile_to_graph(s)
                smile_graph[s] = g
                xd_test.append(s)
                xc_test.append(cell_feature)
                valid_smiles.append(s)
            except Exception:
                pass
                
        if not valid_smiles:
            continue
            
        xd_test = np.asarray(xd_test)
        xc_test = np.asarray(xc_test)
        fake_y = np.zeros(len(xd_test))

        test_data = TestbedDataset(root='/home/cenking/VsCode/GraphDRP/data', dataset='my_infer', xd=xd_test, xt=xc_test, y=fake_y, smile_graph=smile_graph)
        test_loader = DataLoader(test_data, batch_size=128, shuffle=False)
        
        predictions = predicting(model, device, test_loader)
        
        predictions = np.array(predictions)
        eps = 1e-7
        pred_clipped = np.clip(predictions, eps, 1.0 - eps)
        
        ln_ic50 = -10.0 * np.log((1.0 - pred_clipped) / pred_clipped)
        ic50_uM = np.exp(ln_ic50)
        
        out_df = pd.DataFrame({
            'SMILES': valid_smiles,
            'Pred_Normalized_Score': predictions,
            'Pred_ln_IC50': ln_ic50,
            'Pred_IC50_uM': ic50_uM
        })
        out_df.to_csv(output_csv_path, index=False)
        
    print(f"\\nAll prediction files comprehensively processed and saved to {OUTPUT_FOLDER}!")
    
run_batch_prediction()"""

nb.cells = [
    nbf.v4.new_markdown_cell(md1),
    nbf.v4.new_code_cell(code1),
    nbf.v4.new_markdown_cell(md2),
    nbf.v4.new_code_cell(code2),
    nbf.v4.new_markdown_cell(md3),
    nbf.v4.new_code_cell(code3),
    nbf.v4.new_markdown_cell(md4),
    nbf.v4.new_code_cell(code4)
]

nbf.write(nb, '/home/cenking/VsCode/GraphDRP/GraphDRP_AutoRunning.ipynb')

